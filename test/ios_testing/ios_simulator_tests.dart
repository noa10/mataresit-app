...
void main() {
  ...
  group('iOS Simulator Tests', () {
    ...
    group('iOS Version Compatibility Tests', () {
      testWidgets('iOS 14.0+ Minimum Version Support', (WidgetTester tester) async {
        ...
        expect(IOSTestFramework.meetsMinimumIOSVersion(14, 0), isTrue,
            reason: 'App requires iOS 14.0 or later');
        
        logger.i('iOS version compatibility verified: ${IOSTestFramework.iOSVersion}');
      });

      testWidgets('iOS System Features Availability', (WidgetTester tester) async {
        ...
        expect(deviceCategory, isNot(DeviceCategory.unknown),
    ...
    group('iOS UI/UX Adaptation Tests', () {
      testWidgets('iOS Theme Application', (WidgetTester tester) async {
        ...
        if (Platform.isIOS) {
          ...
          expect(materialApp.theme, isNotNull, reason: 'iOS theme should be applied');
    ...
    group('iOS Permissions Testing', () {
      testWidgets('Camera Permission Flow', (WidgetTester tester) async {
        ...
        try {
          final hasPermission = await IOSPermissionsService.requestCameraPermission(context);
          logger.i('Camera permission result: $hasPermission');
          
          // In test environment, this might return false due to mocking
          expect(hasPermission, isA<bool>(), reason: 'Should return boolean result');
        } catch (e) {
          logger.w('Camera permission test failed (expected in test environment): $e');
        }
      });
      ...
      testWidgets('Photo Library Permission Flow', (WidgetTester tester) async {
        ...
        if (!Platform.isIOS) {
          logger.w('Skipping photo library permission test - not running on iOS');
          return;
        }
        ...
        try {
          ...
          logger.w('Photo library permission test failed (expected in test environment): $e');
        }
      });
      ...
      testWidgets('Biometric Permission Flow', (WidgetTester tester) async {
        ...
        try {
          final hasPermission = await IOSPermissionsService.requestBiometricPermission(context);
          logger.i('Biometric permission result: $hasPermission');
          
          expect(hasPermission, isA<bool>(), reason: 'Should return boolean result');
        } catch (e) {
          logger.w('Biometric permission test failed (expected in test environment): $e');
        }
      });
    });
