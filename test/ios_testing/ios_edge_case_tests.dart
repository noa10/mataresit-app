...
void main() {
  ...
  group('iOS Edge Case Tests', () {
    ...
    group('Permission Edge Cases', () {
      testWidgets('Permission Denied Scenarios', (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(body: Text('Permission Test')),
          ),
        );
        ...
        try {
          final cameraResult = await IOSPermissionsService.requestCameraPermission(context);
          logger.i('Camera permission result: $cameraResult');
          
          // Should handle denial gracefully
          expect(cameraResult, isA<bool>(), reason: 'Should return boolean result');
        } catch (e) {
        ...
        try {
          final photoResult = await IOSPermissionsService.requestPhotoLibraryPermission(context);
          logger.i('Photo library permission result: $photoResult');
          
          expect(photoResult, isA<bool>(), reason: 'Should return boolean result');
        } catch (e) {
          logger.i('Photo library permission denial handled: $e');
        }
      });

      testWidgets('Permission Restricted Scenarios', (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(body: Text('Restriction Test')),
          ),
        );
        await tester.pumpAndSettle();
      ...
      testWidgets('Settings Redirection Flow', (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          const MaterialApp(
            home: Scaffold(body: Text('Settings Test')),
          ),
        );
        await tester.pumpAndSettle();
    ...
    group('Apple Sign-In Edge Cases', () {
      testWidgets('Apple Sign-In Availability Check', (WidgetTester tester) async {
        ...
        try {
          ...
          expect(isAvailable, isA<bool>(), reason: 'Should return availability status');
        } catch (e) {
      ...
      testWidgets('Apple Sign-In Cancellation', (WidgetTester tester) async {
        ...
        if (!Platform.isIOS) {
          logger.w('Skipping Apple Sign-In cancellation test - not running on iOS');
          return;
        }
        ...
        try {
          ...
          logger.i('Apple Sign-In result: ${result != null ? 'Success' : 'Cancelled/Failed'}');
      ...
      testWidgets('Apple Sign-In Credential State Changes', (WidgetTester tester) async {
    ...
    group('Keychain Security Edge Cases', () {
      testWidgets('Keychain Access Failures', (WidgetTester tester) async {
        ...
        try {
          ...
          logger.i('Keychain operation result: ${retrieved != null ? 'Success' : 'Failed'}');
      ...
      testWidgets('Biometric Authentication Failures', (WidgetTester tester) async {
        ...
        try {
          await IOSKeychainService.storeBiometricProtectedData('bio_key', 'bio_value');
          final retrieved = await IOSKeychainService.getBiometricProtectedData('bio_key');
          logger.i('Biometric operation result: ${retrieved != null ? 'Success' : 'Failed'}');
    ...
    group('iOS Sharing Edge Cases', () {
      ...
      testWidgets('Large File Sharing', (WidgetTester tester) async {
        ...
        try {
          ...
          final largeData = List.generate(1000000, (index) => 'Large content $index').join('\n');
    ...
    group('App Lifecycle Edge Cases', () {
      testWidgets('App Termination During Operations', (WidgetTester tester) async {
      ...
      testWidgets('Background Processing Limitations', (WidgetTester tester) async {
    ...
    tearDownAll(() async {
      ...
      logger.i('Edge case test logs exported: ${logData['totalEntries']} entries');
      
      // Summary of edge case testing
      logger.i('iOS Edge Case Testing Summary:');
      logger.i('✅ Permission edge cases: Denial, restriction, settings redirection');
      logger.i('✅ Apple Sign-In edge cases: Availability, cancellation, credential changes');
      logger.i('✅ Keychain security edge cases: Access failures, biometric failures, migration');
      logger.i('✅ iOS sharing edge cases: Service unavailable, large files, invalid formats');
      logger.i('✅ App lifecycle edge cases: Termination, memory pressure, background limits');
      logger.i('✅ Network edge cases: Connectivity changes, rate limiting, offline scenarios');
    });
  });
}
