...
class IOSSharingService {
  ...
  static Future<void> shareApp(
    ...
    try {
      const appStoreUrl =
          'https://apps.apple.com/app/mataresit/id123456789'; // Replace with actual App Store URL
      const shareText =
          'Check out Mataresit - the smart receipt manager that makes expense tracking effortless!\n\n$appStoreUrl';
  ...
  static String _buildReceiptShareText(ReceiptModel receipt) {
    ...
    buffer.writeln(
      'Date: ${receipt.transactionDate?.toString().split(' ')[0] ?? 'Unknown Date'}',
    );
    buffer.writeln(
      'Amount: ${receipt.currency ?? 'USD'} ${receipt.totalAmount?.toStringAsFixed(2) ?? '0.00'}',
    );
  ...
  static String _buildMultipleReceiptsShareText(
    List<ReceiptModel> receipts, {
    String? title,
  }) {
    final buffer = StringBuffer();
  ...
  static Future<XFile> _generateReceiptsPDF(
    List<ReceiptModel> receipts, {
    String? title,
  }) async {
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            pw.Header(level: 0, child: pw.Text(title ?? 'Expense Report')),
            pw.TableHelper.fromTextArray(
              headers: ['Date', 'Merchant', 'Amount', 'Category'],
              data: receipts
                  .map(
                    (receipt) => [
                      receipt.transactionDate?.toString().split(' ')[0] ??
                          'Unknown Date',
  ...
  static Future<XFile> _generateClaimPDF(
    ClaimModel claim,
    List<ReceiptModel> receipts,
  ) async {
    final pdf = pw.Document();
    ...
    final output = await getTemporaryDirectory();
    final file = File(
      '${output.path}/claim_${claim.id}_${DateTime.now().millisecondsSinceEpoch}.pdf',
    );
    await file.writeAsBytes(await pdf.save());
  ...
  static Future<XFile> _generateReceiptsCSV(
    List<ReceiptModel> receipts, {
    String? title,
  }) async {
    final buffer = StringBuffer();
    ...
    final output = await getTemporaryDirectory();
    final file = File(
      '${output.path}/receipts_${DateTime.now().millisecondsSinceEpoch}.csv',
    );
