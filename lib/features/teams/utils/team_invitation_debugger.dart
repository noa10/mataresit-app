...
class TeamInvitationDebugger {
  ...
  static Future<void> _checkTeamMembership(String teamId) async {
    ...
    try {
      ...
      AppLogger.debug(
        '✅ Team membership found: ${user.id} has role $role in team $teamId',
      );

      // Check if role allows viewing invitations
      final canViewInvitations = ['owner', 'admin'].contains(role);
      AppLogger.debug(
        '${canViewInvitations ? '✅' : '❌'} Can view invitations: $canViewInvitations',
      );
    } catch (error) {
  ...
  static Future<void> _testDatabaseFunction(String teamId) async {
    ...
    try {
      final response = await SupabaseService.client.rpc(
        ...
        params: {'_team_id': teamId, '_status': null, '_include_expired': true},
      );

      AppLogger.debug(
        '✅ Database function works: ${response?.length ?? 0} invitations returned',
      );
  ...
  static Future<void> _testDirectTableQuery(String teamId) async {
    ...
    try {
      ...
      AppLogger.debug(
        '✅ Direct table query works: ${response.length} invitations returned',
      );
  ...
  static Future<void> _checkExistingInvitations(String teamId) async {
    ...
    try {
      ...
      AppLogger.debug(
        '📊 Invitation statistics: ${response.length} total invitations',
      );
  ...
  static Future<void> _testServiceLayer(String teamId) async {
    ...
    try {
      ...
      AppLogger.debug(
        '✅ Service layer works: ${invitations.length} invitations loaded',
      );
